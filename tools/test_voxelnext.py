...
from eval_utils import eval_utils, voxelnext_eval_utils
...
def parse_config():
  ...
  parser.add_argument('--infer_time', action='store_true',
                      default=False, help='calculate inference latency')
...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False):
  ...
  # start evaluation
  voxelnext_eval_utils.eval_one_epoch(
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False):
  ...
  while True:
    ...
    # start evaluation
    cur_result_dir = eval_output_dir / ('epoch_%s' %
                                        cur_epoch_id) / cfg.DATA_CONFIG.DATA_SPLIT['test']
...
def main():
  ...
  print(cfg.MODEL)
  model = VoxelNext(cfg.MODEL, cfg.CLASS_NAMES, test_set.point_feature_encoder.num_point_features,
                    test_set.grid_size, test_set.point_cloud_range, test_set.voxel_size)
  # exit()
  with torch.no_grad():
    if args.eval_all:
