      ...
      for k in range(num_conv - 1):
        fc_list.append(spconv.SparseSequential(
            spconv.SubMConv2d(
                ...
                bias=use_bias, indice_key=cur_name),
            nn.BatchNorm1d(input_channels),
            nn.ReLU()))
      fc_list.append(spconv.SubMConv2d(input_channels, output_channels,
                     1, bias=True, indice_key=cur_name + 'out'))
...
class VoxelNeXtHead(nn.Module):
  def __init__(self, model_cfg, num_class, class_names, grid_size, point_cloud_range, voxel_size,
    ...
    if self.iou_branch:
      ...
      self.nms_configs = [
          EasyDict(
              NMS_TYPE=nms_configs.NMS_TYPE, NMS_THRESH=nms_configs.NMS_THRESH
              [i],
              NMS_PRE_MAXSIZE=nms_configs.NMS_PRE_MAXSIZE[i],
              NMS_POST_MAXSIZE=nms_configs.NMS_POST_MAXSIZE[i])
    ...
    assert total_classes == len(
      self.class_names), f'class_names_each_head={self.class_names_each_head}'
  ...
  def assign_target_of_single_head(
    ...
    for k in range(min(num_max_objs, gt_boxes.shape[0])):
      ...
      if not (
              0 <= center_int[k][0] <= spatial_shape[1] and 0 <= center_int[k][1] <= spatial_shape[0]):
        continue
      ...
      if 'gt_center' in self.gaussian_type:
        centernet_utils.draw_gaussian_to_heatmap_voxels(
            ...
            distance, radius[k].item() * self.gaussian_ratio)

      if 'nearst' in self.gaussian_type:
        centernet_utils.draw_gaussian_to_heatmap_voxels(heatmap[cur_class_id], self.distance(
          spatial_indices, spatial_indices[inds[k]]), radius[k].item() * self.gaussian_ratio)
  ...
  def get_loss(self):
    ...
    for idx, pred_dict in enumerate(pred_dicts):
      ...
      pred_boxes = torch.cat([pred_dict[head_name]
                             for head_name in self.separate_head_cfg.HEAD_ORDER], dim=1)

      reg_loss = self.reg_loss_func(
          pred_boxes, target_dicts['masks'][idx],
          target_dicts['inds'][idx],
          target_boxes, batch_index)
      loc_loss = (
        reg_loss * reg_loss.new_tensor(self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['code_weights'])).sum()
      ...
      if self.iou_branch:
        ...
        iou_reg_loss = self.crit_iou_reg(
            batch_box_preds, target_dicts['masks'][idx],
            target_dicts['inds'][idx],
            target_dicts['gt_boxes'][idx],
            batch_indices)
        iou_weight = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS[
          'iou_weight'] if 'iou_weight' in self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS else self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['loc_weight']
  ...
  def _get_predicted_boxes(self, pred_dict, spatial_indices):
    ...
    xs = (spatial_indices[:, 1:2] + center[:, 0:1]
          ) * self.feature_map_stride * self.voxel_size[0] + self.point_cloud_range[0]
    ys = (spatial_indices[:, 0:1] + center[:, 1:2]
          ) * self.feature_map_stride * self.voxel_size[1] + self.point_cloud_range[1]
  ...
  def rotate_class_specific_nms_iou(self, boxes, scores, iou_preds, labels, rectifier, nms_configs):
    ...
    for cls in range(self.num_class):
      ...
      scores_cls = torch.pow(scores[mask], 1 - rectifier[cls]
                             ) * torch.pow(iou_preds[mask].squeeze(-1), rectifier[cls])
    ...
    return torch.cat(
        box_preds_list, dim=0), torch.cat(
        scores_list, dim=0), torch.cat(
        labels_list, dim=0)
  ...
  def merge_double_flip(self, pred_dict, batch_size, voxel_indices, spatial_shape):
    ...
    for bs_idx in range(batch_size):
      ...
      spatial_indices_unique, _inv, count = torch.unique(
        spatial_indices_batch, dim=0, return_inverse=True, return_counts=True)
  ...
  def generate_predicted_boxes(self, batch_size, pred_dicts, voxel_indices, spatial_shape):
    ...
    for k in range(batch_size):
      ...
      if self.iou_branch:
        ...
        pred_boxes, pred_scores, pred_labels = self.rotate_class_specific_nms_iou(
          pred_boxes, pred_scores, pred_ious, pred_labels, self.rectifier, self.nms_configs)
  ...
  def forward(self, batch_size, encoded_spconv_tensor, gt_boxes=None):
    spatial_shape, batch_index, voxel_indices, spatial_indices, num_voxels = self._get_voxel_infos(
        encoded_spconv_tensor)
    self.forward_ret_dict['batch_index'] = batch_index

    pred_dicts = []
    for head in self.heads_list:
      pred_dicts.append(head(encoded_spconv_tensor))

    if self.training:
      target_dict = self.assign_targets(
          gt_boxes, num_voxels, spatial_indices, spatial_shape
      )
